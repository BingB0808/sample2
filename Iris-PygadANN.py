...
def classify(sl,sw,pl,pw, si=0):
    global GANN_instance, data_outputs
    data_inputs = [[sl,sw,pl,pw]]
   
    predictions = pygad.nn.predict(last_layer=GANN_instance.population_networks[si],
...
def fitness_func(solution, sol_idx):
    ...
    oo = [i == j for i, j in zip(predictions, data_outputs)]
    
    solution_fitness = sum(oo)/len(data_outputs)*100
...
data_outputs = iris['variety']

data_outputs  = data_outputs.tolist()
...
wrong = [i == j for i, j in zip(predictions, data_outputs)]
num_wrong = sum(wrong)
num_correct = len(data_outputs) - num_wrong.size
accuracy = 100 * (num_correct/len(data_outputs))
...
pickle.dump(elast_layer,open('GA1.pkl','wb'))
